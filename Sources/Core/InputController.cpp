...
void InputController::MouseScrolled(float x, float y)
{
	mScrollDelta = Vector2d<float>{ x, y };
	mMouseScrolledThisFrame = true;
}

void InputController::KeyReleased(Key key)
{
	mPressedKeys.erase(key);
}

void InputController::KeyPressed(Key key)
{
	mPressedKeys.insert(key);
}

std::set<InputController::Key> InputController::GetPressedKeys()
{
	return mPressedKeys;
}

Vector2d<float> InputController::GetMouseDelta()
{
...
Vector2d<float> InputController::GetScrollDelta()
{
